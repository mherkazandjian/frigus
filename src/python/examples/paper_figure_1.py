...
#species_data_wrathmall = DataLoader().load('H2_low_energy_levels')

plt.ion()
fig, axs = plt.subplots(figsize=(4, 4))
axs.set_position([0.16, 0.1, 0.8, 0.8])

# density of the colliding species, in m^3
nc_h_rng = [
               1.e2, 1.e3, 1.e4, 1.e5, 1.e6, 1.e7, 1.e8, 1.e9,
               1.e10, 1.e11, 1.e12, 1.e13, 1.e14
           ] * u.meter ** -3
...
for nc_index, nc_h in enumerate(nc_h_rng):
    ...
    for i, t_kin in enumerate(t_rng):
        ...
        m_h2_h = compute_transition_rate_matrix(
            species_data_h2_h,
            t_kin,
            t_rad,
            nc_h)
        ...
        m_h2_wrathmall = compute_transition_rate_matrix(
    ...
    axs.set_xlabel('T$_\mathrm{kin}$ [K]', fontsize=12)
    axs.set_ylabel('cooling function [erg $\cdot$ s$^{-1}$]', fontsize=12)

    # axs.loglog(
    #     t_rng.value, lambda_vs_t_kin_wrathmall.to(u.erg / u.second).value,
    #      '.', color='red', marker=plot_markers[nc_index]
    # )
...
axs.set_yticks(axs.get_yticks()[::2])

axs.set_xticks(
    [100.0, 200.0, 300.0, 400.0, 500.0, 600.0, 700.0, 800.0, 900.0, 1000.0, 2000.0, 3000]
)
