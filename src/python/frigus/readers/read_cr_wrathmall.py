...
#    read_cr_wrathmall.py is part of Frigus.
...
class Reader(object):
    ...
    def read_data(self):
        ...
        # temporary lists to store kinetic temperatures, levels and rates for
        # all the block
        tkin, levels, rates = [], [], []
    ...
    def info(self):
        ...
        print('\t {} {} {}'.format(unq_ef.size, unq_v.size, unq_j.size))
        ...
        for i in numpy.arange(self.data.shape[0]):
            for j in numpy.arange(self.data.shape[1]):
                for k in numpy.arange(self.data.shape[2]):
                    if self.data[i, j, k] is not None:
                        n_bytes += self.data[i, j, k].nbytes
        print(
            'total number of bytes in memory: {:2.f} MB'.format(
                (numpy.float64(n_bytes) / 1024.0 ** 2.0)
            )
        )

        # plot the available transitions irrespective of the fina electronic
        # state
